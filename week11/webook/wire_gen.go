...
func InitWebServer() *App {
	...
	producer := article.NewSaramaSyncProducer(syncProducer, loggerV1)
	...
	identity := ioc.NewIdentity()
	decider := loaddecider.NewRedisZSetDecider(cmdable, loggerV1, identity)
	rankingJob := ioc.InitRankingJob(rankingService, rlockClient, decider, loggerV1)
	cron := ioc.InitJobs(loggerV1, rankingJob)
	reporter := loaddecider.NewRedisZSetReporter(cmdable)
	gauge := loaddecider.NewDefaultLoadGauge(loggerV1, reporter, identity)
	app := &App{
		server:    engine,
		consumers: v2,
		cron:      cron,
...
var deciderSet = wire.NewSet(loaddecider.NewRedisZSetReporter, ioc.NewIdentity, loaddecider.NewDefaultLoadGauge, loaddecider.NewRedisZSetDecider)
